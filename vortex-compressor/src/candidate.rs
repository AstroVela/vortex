...
pub struct Candidate<'a> {
    ...
    /// The scheme's estimated compression ratio, if one exists.
...
impl<'a> Candidate<'a> {
    ...
    /// Returns the estimated compression ratio, or `None` for a zero-byte sampled output.
    pub fn estimated_ratio(&self) -> Option<f64> {
