...
impl MorselScan {
    ...
    fn validate_morsels(&self) -> VortexResult<()> {
        ...
        for range in self.morsels.iter() {
            ...
            if (!self.sparse_morsels && range.start != previous_end)
                || (self.sparse_morsels && range.start < previous_end)
            {
                return Err(vortex_err!(
        ...
        if !self.sparse_morsels && previous_end != row_count {
            return Err(vortex_err!("morsel cut does not cover the plan"));
        }
        Ok(())
    }
    ...
    ///
    /// This is used by scan integrations after selection and statistics pruning.
