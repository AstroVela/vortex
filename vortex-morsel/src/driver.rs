...
/// A persistent worker set shared by successive scans.
///
/// Runs are serialized because each scan dispatches work to every affinity worker. Each worker
/// retains a separate arena for every plan it encounters, so adaptive ordering state remains local
/// to that worker and plan.
...
impl SharedMorselWorkerPool {
    ...
    /// The number of affinity workers available to each scan.
...
impl MorselWorkerPool {
    fn new(threads: usize) -> VortexResult<Self> {
        ...
        for idx in 0..threads {
            ...
            let handle = std::thread::Builder::new()
                ...
                .spawn(move || {
                    let mut arenas: HashMap<usize, (Arc<ExecPlan>, Arena)> = HashMap::default();
                    ...
                    while let Ok(message) = message_rx.recv() {
                        match message {
                            WorkerMessage::Run {
                                ...
                                let plan_key = Arc::as_ptr(&plan) as usize;
                                let (_, arena) = arenas
                                    .entry(plan_key)
                                    .or_insert_with(|| (Arc::clone(&plan), plan.instantiate()));
                                let mut execution = scheduler.run.session.create_execution_ctx();
                                let stats =
                                    scheduler.worker_loop(worker, &signals, arena, &mut execution);
    ...
    fn run(
        ...
        let active_workers = signals.len();
        ...
        let mut stats = Vec::with_capacity(active_workers);
...
impl MorselScan {
    ...
    /// Drive this scan with workers whose lifecycle spans successive scans.
    pub fn with_worker_pool(mut self, worker_pool: Arc<SharedMorselWorkerPool>) -> Self {
        self.threads = self.threads.min(worker_pool.threads());
    ...
    pub(crate) fn run_timed(&self) -> VortexResult<(Vec<ArrayRef>, ScanStats, Duration)> {
        ...
        if self.worker_pool.is_none() && workers.is_none() {
            *workers = Some(MorselWorkerPool::new(self.threads)?);
        }
        ...
        let worker_stats = match (&self.worker_pool, workers.as_ref()) {
            (Some(worker_pool), _) => worker_pool.run(Arc::clone(&scheduler), signals)?,
            (None, Some(workers)) => workers.run(Arc::clone(&scheduler), signals)?,
            (None, None) => return Err(vortex_err!("morsel worker pool was not initialized")),
        };
