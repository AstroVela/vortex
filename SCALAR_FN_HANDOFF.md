...
emitted `nvptx64` PTX for a row body, and the layering that separates it from the encoding decode
path. It contains no timings. All four are branch-only working notes for agents. They are not
intended to land with the API.
