...
impl<A: 'static + Send> ScanBuilder<A> {
    ...
    /// Selects absolute row indices relative to the scan input.
    ...
    pub fn prepare(self) -> VortexResult<RepeatedScan<A>> {
        ...
        let source = RowIdxPlan::new_ref(self.row_offset, Arc::clone(&self.base_plan));
        tracing::debug!(
            target: "vortex_scan_v2::planner",
            row_offset = self.row_offset,
            plan = %source.tree_display(),
...
fn optimize_projection_plan(expression: Expression, source: &PlanRef) -> VortexResult<PlanRef> {
    ...
    let expression = expression.optimize_recursive(source.dtype())?;
    let projection: PlanRef = Arc::new(ExpressionPlan::try_new(expression, Arc::clone(source))?);
    let projection = projection.optimize()?;
    tracing::debug!(
        target: "vortex_scan_v2::planner",
        plan = %projection.tree_display(),
...
fn optimize_filter_expression(
    ...
    filter
        .map(|expression| {
            ...
            expression.optimize_recursive(source.dtype())
        })
        .transpose()
}
...
fn optimize_pruning_plan(
    ...
    let Some(falsifier) = filter.falsify(source.dtype(), session)? else {
        tracing::debug!(
            target: "vortex_scan_v2::planner",
            expression = %filter,
    ...
    let pruning: PlanRef = Arc::new(ExpressionPlan::try_new(falsifier, Arc::clone(source))?);
    let pruning = pruning.optimize()?;
    vortex_ensure!(
...
fn optimize_filter_plan(
    ...
    let filter: PlanRef = Arc::new(ExpressionPlan::try_new(
        expression.clone(),
        Arc::clone(source),
    )?);
...
fn uses_only_pruning_sources(plan: &PlanRef) -> VortexResult<bool> {
    if let Some(zoned) = plan.downcast_ref::<ZonedPlan>() {
        return Ok(zoned.is_pruning());
    }
    if plan.is::<RowIdxValuesPlan>() {
        return Ok(true);
    }
    ...
    for index in 0..plan.child_count() {
        let Some(child) = plan.child(index)? else {
            continue;
        };
        found_child = true;
