...
impl<A: 'static + Send> ScanBuilder<A> {
    ...
    pub fn prepare(self) -> VortexResult<RepeatedScan<A>> {
        ...
        let projection = self.projection.optimize_recursive(source.dtype())?;
        let projection: PlanRef =
            Arc::new(ExpressionPlan::try_new(projection, Arc::clone(&source))?);
        ...
        let filter = self
            ...
            .map(|filter| -> VortexResult<PlanRef> {
                ...
                let filter = filter.optimize_recursive(source.dtype())?;
                let filter: PlanRef =
                    Arc::new(ExpressionPlan::try_new(filter, Arc::clone(&source))?);
