...
impl VTable for RunEnd {
    ...
    fn validate(
        ...
        let s = RunEndSlotsView::from_slots(slots);
        // TODO(ctx): trait fixes - VTable::validate has a fixed signature.
        let mut ctx = legacy_session().create_execution_ctx();
        RunEndData::validate_parts(s.ends, s.values, data.offset, len, &mut ctx)?;
        vortex_ensure!(
...
pub trait RunEndArrayExt: TypedArrayRef<RunEnd> + RunEndArraySlotsExt {
