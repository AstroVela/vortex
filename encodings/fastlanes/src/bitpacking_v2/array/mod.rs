...
use vortex_array::patches_v2::PatchMode;
...
pub struct BitPackedV2Data {
    ...
    /// Patch metadata for reconstructing PatchesV2 from slots.
    pub(super) patches_data: Option<PatchesV2Data>,
}
...
impl BitPackedV2Data {
    ...
    pub fn try_new(
        ...
        Ok(Self {
            ...
            packed,
            bit_widths,
            patches_data: patches.as_ref().map(PatchesV2Data::from_patches),
        })
    }
    ...
    pub(crate) fn validate(
        ...
        if let Some(patches) = patches {
            vortex_ensure!(
                patches.indices().dtype()
                    == &DType::Primitive(PType::U16, Nullability::NonNullable),
                "BitPackedV2Array patch indices must be non-nullable u16",
            );
            vortex_ensure!(
                patches.chunk_offsets().dtype()
                    == &DType::Primitive(PType::U32, Nullability::NonNullable),
                "BitPackedV2Array patch chunk offsets must be non-nullable u32",
            );
            vortex_ensure!(
                patches.indices().len() == patches.values().len(),
                "BitPackedV2Array patch indices and values must have the same length",
            );
            vortex_ensure!(
                patches.chunk_offsets().len() == expected_chunks + 1,
                "BitPackedV2Array expected {} patch chunk offsets, got {}",
                expected_chunks + 1,
                patches.chunk_offsets().len(),
            );
            vortex_ensure!(
            ...
            vortex_ensure!(
                patches.offset() == self.offset as usize,
                "PatchesV2 offset {} does not match BitPackedV2Array offset {}",
                patches.offset(),
                self.offset,
            );
            vortex_ensure!(
                patches.mode() == PatchMode::Replace,
                "BitPackedV2Array requires replace-mode patches",
            );
        }
...
pub trait BitPackedV2ArrayExt: BitPackedV2ArraySlotsExt {
    ...
    #[inline]
    fn patches(&self) -> Option<PatchesV2> {
        PatchesV2Data::patches_from_slots(
