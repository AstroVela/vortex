...
) -> VortexResult<(ByteBuffer, Buffer<u8>, Option<PatchesV2>)> {
...
fn gather_patches<T: NativePType + PrimInt>(
    ...
    ptype: PType,
    patch_validity: Validity,
) -> VortexResult<Option<PatchesV2>> {
    match validity_mask.bit_buffer() {
        // All-null arrays never patch: every value is undefined.
        AllOr::None => Ok(None),
        AllOr::All => gather_patches_with_validity::<T, _>(
            values,
            bit_widths,
        ...
        AllOr::Some(buffer) => gather_patches_with_validity::<T, _>(
            values,
            bit_widths,
            buffer.iter(),
            ptype,
...
fn gather_patches_with_validity<T: NativePType + PrimInt, I: Iterator<Item = bool>>(
    values: &[T],
    bit_widths: &[u8],
    mut validity: I,
    ptype: PType,
    ...
    let mut indices = BufferMut::<u16>::empty();
    let mut patch_values = BufferMut::<T>::empty();
    let mut chunk_offsets = BufferMut::<u32>::with_capacity(bit_widths.len() + 1);
    ...
    for (chunk_idx, chunk) in values.chunks(FL_CHUNK_SIZE).enumerate() {
        ...
        for (idx, &value) in chunk.iter().enumerate() {
            ...
            if is_valid && value_bit_width(value) > width {
                indices.push(u16::try_from(idx)?);
                patch_values.push(value);
            }
        }
        chunk_offsets.push(u32::try_from(patch_values.len())?);
    }
    ...
    // SAFETY: indices are emitted in strictly increasing order within each chunk, every index is
    // local to a 1024-value chunk, and the prefix offsets start at zero and end at the patch count.
    Ok(Some(unsafe {
