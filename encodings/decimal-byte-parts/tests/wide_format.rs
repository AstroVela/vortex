...
//! The wide serialized format.
//!
//! Lower parts can be built and computed over freely. What changes with them is the bytes:
//! an array carrying lower parts serializes under `vortex.decimal_byte_parts_wide` rather
//! than the frozen `vortex.decimal_byte_parts` format, so a writer restricted to editions
//! without the wide format refuses it, and a reader that predates lower parts fails with an
...
use vortex_decimal_byte_parts::DecimalBytePartsWide;
...
/// the format froze; lower parts move the array onto the wide format id.
#[test]
...
/// frozen format — a writer whose enabled editions predate the wide format — must refuse an
...
fn wide_format_is_refused_where_not_permitted() -> VortexResult<()> {
    ...
    // Permitting the wide format id is exactly what allows the same array through.
    ...
    assert!(
        permissive
            .to_ids()
            .contains(&ArrayPlugin::id(&DecimalBytePartsWide)),
        "the file's encoding table must carry the wide format id"
    );
