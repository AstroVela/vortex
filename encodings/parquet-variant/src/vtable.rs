...
use crate::array::ParquetVariantSlotsView;
...
impl VTable for ParquetVariant {
    ...
    fn validate(
        ...
        let ParquetVariantSlotsView {
            validity,
            metadata,
            value,
            typed_value,
        } = ParquetVariantSlotsView::from_slots(slots);
        let validity = child_to_validity(validity, dtype.nullability());
