...
pub struct ListViewSlots {
    ...
    /// we want to access.
    pub sizes: ArrayRef,
    /// The validity bitmap indicating which lists are non-null.
...
impl Array<ListView> {
    ...
    pub fn into_data_parts(self) -> ListViewDataParts {
        ...
        let view = ListViewSlotsView::from_slots(self.slots());
        ListViewDataParts {
            elements_dtype: Arc::new(view.elements.dtype().clone()),
            elements: view.elements.clone(),
            offsets: view.offsets.clone(),
            sizes: view.sizes.clone(),
            validity,
        }
    }
}
