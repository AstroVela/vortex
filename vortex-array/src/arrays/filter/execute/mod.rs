...
/// Check for the fast-path execution conditions that do not depend on the selection mask.
///
/// The mask fast paths are matched by the caller, which needs the [`MaskValuesRef`] of a partially
/// selective mask anyway.
pub(super) fn execute_filter_fast_paths(
    array: ArrayView<'_, Filter>,
    true_count: usize,
    ctx: &mut ExecutionCtx,
) -> VortexResult<Option<ArrayRef>> {
    // If the array itself is completely null, we only care about the total number of nulls, not
    // the values.
