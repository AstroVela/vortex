...
//! Fused comparison and bit-packing for primitive lanes.
//!
//! Production uses this implementation on targets where the RowFn implementation does not have an
//! explicit SIMD kernel. Tests retain it on every target as the benchmark and semantic baseline.
...
pub(super) fn compare_primitive(
    ...
    match_each_native_ptype!(ptype, |T| {
        compare_primitive_typed::<T>(lhs, rhs, op, ctx)
    })
}
