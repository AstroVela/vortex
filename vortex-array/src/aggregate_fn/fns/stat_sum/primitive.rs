...
/// StatSum the values of a float slice into an `f64` accumulator. When `skip_nans` is set, NaN values
...
/// StatSum all values into a `u64` accumulator. For types narrower than 64 bits, values are summed in
...
/// StatSum the valid elements, described as contiguous `[start, end)` runs of set validity bits. Each
...
mod tests {
    ...
    /// StatSum an array with explicit [`NumericalAggregateOpts`] (test-only helper).
    fn sum_with_options(
        arr: &crate::ArrayRef,
        options: NumericalAggregateOpts,
    ) -> VortexResult<Scalar> {
        let mut acc = Accumulator::try_new(StatSum, options, arr.dtype().clone())?;
    ...
    fn sum_not_skipping_uses_cached_sum_when_nan_free() -> VortexResult<()> {
        // With an exact NaNCount of zero, the planted exact StatSum stat is usable as-is.
        let arr =
            PrimitiveArray::new(buffer![1.0f64, 2.0, 3.0], Validity::NonNullable).into_array();
        arr.statistics()
    ...
    fn sum_f64_with_infinity() -> VortexResult<()> {
        ...
        .into_array();
        let acc = stat_sum(&batch, &mut array_session().create_execution_ctx())?;
