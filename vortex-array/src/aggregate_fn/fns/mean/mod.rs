...
/// Compute the arithmetic mean of an array.
///
/// Implemented as `Total / Count` via [`BinaryCombined`].
///
...
impl BinaryCombined for Mean {
    ...
    fn finalize_scalar(&self, left_scalar: Scalar, right_scalar: Scalar) -> VortexResult<Scalar> {
        ...
        let value = match (sum, count) {
            (None, _) | (_, None) => return Ok(Scalar::null(target)), // Total overflowed
    ...
    fn coerce_args(
        ...
        _options: &PairOptions<
            <Total as AggregateFnVTable>::Options,
...
///
/// - Bool stays as bool — `Total` has a native bool path and bool → f64 isn't
