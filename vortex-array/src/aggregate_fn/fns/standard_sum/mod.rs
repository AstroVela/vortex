...
mod tests {
    ...
    mod bool_inputs {
        ...
        fn sum_bool_empty_is_null() -> VortexResult<()> {
            ...
            let mut acc =
                Accumulator::try_new(StandardSum, NumericalAggregateOpts::default(), dtype)?;
            let result = acc.finish()?;
            assert!(result.is_null());
            Ok(())
        }
    ...
    mod constant_inputs {
        ...
        fn sum_constant_nullable_value() -> VortexResult<()> {
            let array =
                ConstantArray::new(Scalar::null(DType::Primitive(PType::U32, Nullable)), 10)
                    .into_array();
            let result = standard_sum(&array, &mut array_session().create_execution_ctx())?;
        ...
        fn sum_constant_decimal_null() -> VortexResult<()> {
            let decimal_dtype = DecimalDType::new(10, 2);
            let array =
                ConstantArray::new(Scalar::null(DType::Decimal(decimal_dtype, Nullable)), 10)
                    .into_array();
    ...
    mod decimal_inputs {
        ...
        fn sum_decimal_near_precision_boundary() -> VortexResult<()> {
            ...
            let small =
                Scalar::decimal(DecimalValue::from(9i64), DecimalDType::new(14, 0), Nullable);
    ...
    mod primitive_inputs {
        ...
        fn sum_u8() -> VortexResult<()> {
            let arr =
                PrimitiveArray::new(buffer![10u8, 20, 30], Validity::NonNullable).into_array();
            let result = standard_sum(&arr, &mut array_session().create_execution_ctx())?;
        ...
        fn sum_f32_with_nan() -> VortexResult<()> {
            let arr = PrimitiveArray::new(buffer![1.0f32, f32::NAN, 4.0], Validity::NonNullable)
                .into_array();
            let result = standard_sum(&arr, &mut array_session().create_execution_ctx())?;
        ...
        fn sum_f64_with_nan_and_nulls() -> VortexResult<()> {
            let arr =
                PrimitiveArray::from_option_iter([Some(1.0f64), None, Some(f64::NAN), Some(3.0)])
                    .into_array();
            let result = standard_sum(&arr, &mut array_session().create_execution_ctx())?;
        ...
        fn sum_all_nan() -> VortexResult<()> {
            let arr = PrimitiveArray::new(buffer![f64::NAN, f64::NAN], Validity::NonNullable)
                .into_array();
            let result = standard_sum(&arr, &mut array_session().create_execution_ctx())?;
            assert_eq!(result.as_primitive().typed_value::<f64>(), Some(0.0));
            Ok(())
        ...
        fn sum_f64_with_nan_not_skipping() -> VortexResult<()> {
            let arr = PrimitiveArray::new(buffer![1.0f64, f64::NAN, 2.0], Validity::NonNullable)
                .into_array();
