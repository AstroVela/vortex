...
impl Node for BoundExpression {
    fn apply_children<'a, F: FnMut(&'a Self) -> VortexResult<TraversalOrder>>(
        ...
        let children: &[Self] = match self {
            ...
            | BoundExpression::HigherOrder { children, .. } => children,
    ...
    fn iter_children<T>(&self, f: impl FnOnce(&mut dyn Iterator<Item = &Self>) -> T) -> T {
        match self {
            BoundExpression::Scalar { children, .. }
            | BoundExpression::HigherOrder { children, .. } => f(&mut children.iter()),
            BoundExpression::Lambda { .. }
            | BoundExpression::Root { .. }
    ...
    fn children_count(&self) -> usize {
        match self {
            BoundExpression::Scalar { children, .. }
            | BoundExpression::HigherOrder { children, .. } => children.len(),
            BoundExpression::Lambda { .. }
            | BoundExpression::Root { .. }
