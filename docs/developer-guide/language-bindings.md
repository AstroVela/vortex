...
<!-- CROSS-DOC CONSISTENCY: the Python-vs-C++ migration story below must match docs/project/bindings.md
     (Python stays on PyO3; only C++ plans to wrap the C ABI). This roadmap claim is NOT code-derivable,
     so it is intentionally not conformance-locked — if you change it, update both docs in lockstep. -->
