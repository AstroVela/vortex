...
(x86_64 and aarch64) and macOS (aarch64), and relocated dependencies that avoid conflicts with
Spark. The unclassified thin JAR is not usable by itself.
...
Supported filters are pushed into the Vortex scan. Nested filter pushdown through the V1
fallback also requires `vortex` in
