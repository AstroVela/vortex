-- GitHub Archive table registration, the companion of this benchmark's query files.
--
...
--
-- The one table globs every file in the format directory.
-- Statements are split on semicolons, so a comment must never contain one.

-- @engine duckdb
CREATE {object} IF NOT EXISTS events AS SELECT * FROM {read}('{dir}/*.{ext}');

-- @engine datafusion
CREATE EXTERNAL TABLE IF NOT EXISTS events STORED AS {format} LOCATION '{dir}/*.{ext}';
