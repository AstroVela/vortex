-- SpatialBench queries for DataFusion with GeoDataFusion functions.
-- Geometry columns contain WKB, so each spatial expression decodes its operands.
...
FROM trip t
WHERE ST_Distance(ST_GeomFromWKB(t.t_pickuploc), ST_GeomFromText('POINT (-111.7610 34.8697)')) <= 0.45
...
JOIN (
    ...
    LIMIT 1
) county ON ST_Intersects(
    ST_GeomFromWKB(t.t_pickuploc),
    ST_GeomFromWKB(county.z_boundary)
);
...
FROM trip t
JOIN building b ON ST_Distance(ST_GeomFromWKB(t.t_pickuploc), ST_GeomFromWKB(b.b_boundary)) <= 0.0045
