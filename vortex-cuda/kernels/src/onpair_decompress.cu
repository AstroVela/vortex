...
// dictionary. Each warp owns 128 tokens. Short entries read 8 bytes; long
// entries request a second 8-byte half through a dense per-warp queue. The
// first high-half load overlaps the independent low-byte shared-memory stores.
...
__device__ inline uint32_t pack_high_request(
    uint32_t code, uint32_t destination, uint32_t high_length) {
    return code | (destination << 16u) | ((high_length - 1u) << 28u);
}

__device__ inline void emit_high_bytes(uint8_t *s_buf, uint32_t destination,
                                       uint32_t high_length, const uint2 &high) {
...
void onpair_decompress(
    ...
    __shared__ __align__(16) uint8_t
        s_buf_all[WARPS_PER_BLOCK_MAX * WARP_BUF_BYTES];
    __shared__ __align__(16)
        uint32_t s_requests[WARPS_PER_BLOCK_MAX][REQUESTS_PER_WARP];
...
#pragma unroll
    for (int k = 0; k < 4; ++k) {
        ...
        if (i < total_tokens) {
            ...
            lo[k] = *reinterpret_cast<const uint2 *>(
                dict_s8 + (size_t)code[k] * 8u);
    ...
    static_assert(32u * 16u <= field_mask,
                  "packed fields must hold a full plane prefix");
    uint64_t packed = (uint64_t)len[0] | ((uint64_t)len[1] << 16u) |
                      ((uint64_t)len[2] << 32u) |
                      ((uint64_t)len[3] << 48u);
...
#pragma unroll
    for (int k = 0; k < 4; ++k) {
        ...
        const uint32_t plane_total =
            (uint32_t)((packed_totals >> shift) & field_mask);
...
#pragma unroll
    for (int k = 0; k < 4; ++k) {
        ...
        const uint32_t lower_lanes =
            lane == 0 ? 0u : ((1u << (uint32_t)lane) - 1u);
        const uint32_t rank = __popc(needs_mask & lower_lanes);
        if (needs_high) {
            requests[high_count + rank] = pack_high_request(
                code[k], excl[k] + 8u, len[k] - 8u);
        }
    ...
    if (first_active) {
        ...
        first_high = *reinterpret_cast<const uint2 *>(
            dict_padded + (size_t)selected_code * 16u + 8u);
    }
