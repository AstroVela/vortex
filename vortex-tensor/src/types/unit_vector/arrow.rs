...
//! unit-vector refinement. This keeps import structural and zero-copy. Callers handling untrusted
...
impl ArrowImportVTable for UnitVector {
    ...
    fn from_arrow_field(
        ...
        if !is_supported_float(element.data_type()) || element.is_nullable() || *list_size <= 0 {
            return Ok(None);
        }
    ...
    fn from_arrow_array(
        ...
        // SAFETY: The Arrow extension marker is the producer's UnitVector claim. Exact operations
        // still measure the imported coordinates; approximate operations explicitly trust it.
        let imported = unsafe { UnitVector::new_unchecked(storage)? };

        Ok(ArrowImport::Imported(imported))
    }
}
