...
#[test]
fn checked_constructor_accepts_unit_rows() -> VortexResult<()> {
    let array = unit_vector(2, &[0.6, 0.8, 1.0, 0.0])?;
...
#[test]
fn checked_constructor_rejects_zero_row() {
    assert!(unit_vector(2, &[0.0, 0.0]).is_err());
}
...
#[test]
fn checked_constructor_rejects_non_finite_row() {
    assert!(unit_vector(2, &[f64::NAN, 0.0]).is_err());
}
...
#[test]
fn dtype_rejects_zero_dimensions() {
    assert!(ExtDType::<UnitVector>::try_new(EmptyMetadata, storage_dtype(PType::F32, 0)).is_err());
}
