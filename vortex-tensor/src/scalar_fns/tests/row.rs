...
fn derived_fn_executes_with_nulls() -> VortexResult<()> {
    ...
    let prim: PrimitiveArray = ScalarFnArray::try_new(L1Norm.bind(EmptyOptions), vec![arr])?
        .into_array()
        .execute(&mut ctx)?;
...
fn derived_fn_dispatches_at_input_width() -> VortexResult<()> {
    ...
    let f32_result: PrimitiveArray = ScalarFnArray::try_new(
        L1Norm.bind(EmptyOptions),
        vec![tensor_array(&[2], &[3.0f32, -4.0])?],
    )?
    .into_array()
    .execute(&mut ctx)?;
    ...
    let f64_result: PrimitiveArray = ScalarFnArray::try_new(
        L1Norm.bind(EmptyOptions),
        vec![tensor_array(&[2], &[3.0f64, -4.0])?],
    )?
    .into_array()
    .execute(&mut ctx)?;
