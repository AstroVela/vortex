...
def test_chunked_encode():
    ...
    assert arrow.combine_chunks() == pa.array([0, 1, 2, 3, 4, 5])  # ty: ignore[invalid-argument-type]
...
def test_table_encode():
    ...
    arrow = encoded.to_arrow_array()
    assert isinstance(arrow, pa.ChunkedArray)
    assert arrow.combine_chunks() == pa.StructArray.from_arrays(  # ty: ignore[invalid-argument-type]
