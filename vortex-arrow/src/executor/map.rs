...
pub(super) fn to_arrow_map(
    ...
    let entries = array.entries().into_owned().into_array();
...
mod tests {
    ...
    fn arrow_map_import_sets_zero_copy_to_list() -> VortexResult<()> {
        ...
        assert!(map.entries().into_owned().is_zero_copy_to_list());
        ...
        let map = vortex.as_::<VortexMap>();
        assert!(map.entries().into_owned().is_zero_copy_to_list());
        assert_eq!(map.entries().offset_at(0), 1);
        assert_eq!(map.entries().offset_at(1), 3);
    ...
    fn sliced_arrow_map_import_preserves_nonzero_offsets() -> VortexResult<()> {
        ...
        assert_eq!(map.entries().offset_at(0), 1);
        assert_eq!(map.entries().offset_at(1), 3);
        assert_eq!(map.entries().size_at(0), 2);
        assert_eq!(map.entries().size_at(1), 1);
