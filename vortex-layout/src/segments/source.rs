...
pub trait SegmentSource: 'static + Send + Sync {
    /// Return the expected number of bytes for a segment without issuing a request.
    ///
    /// Sources that cannot determine the size cheaply should return `None`.
    fn estimated_size(&self, _id: SegmentId) -> Option<usize> {
        None
    }
