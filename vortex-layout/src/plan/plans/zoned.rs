...
impl ZonedPlan {
    ...
    fn with_pruning(&self, expression: Expression, dtype: DType) -> Option<Self> {
    ...
    pub fn pruning_expression(&self) -> Option<&Expression> {
...
impl PlanParentReduceRule<ZonedPlan> for ExpressionZonedRule {
    ...
    fn reduce_parent(
        ...
        let mut contains_unbound_root = false;
        parent.expression().clone().transform_down(|expression| {
            ...
            contains_unbound_root |= is_root(&expression);
            Ok(Transformed::no(expression))
        })?;
        if !parent.dtype().is_boolean() || !contains_stat || contains_unbound_root {
            return Ok(None);
        }

        Ok(child
            .with_pruning(parent.expression().clone(), parent.dtype().clone())
