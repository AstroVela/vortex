...
fn stats_expression_rewrites_to_zoned_pruning_plan() -> VortexResult<()> {
    ...
    let bound_falsifier = falsifier.optimize_recursive(&dtype)?.bind(&dtype)?;
    let plan: PlanRef = Arc::new(ExpressionPlan::new(
        bound_falsifier.clone(),
        Arc::clone(&source),
    ));
    ...
    assert_eq!(zoned.pruning_expression(), Some(&bound_falsifier));
    assert!(zoned.child(0)?.is_none());

    let mixed = make_expression_plan(and(falsifier, gt(root(), lit(0_i32))), source)?.optimize()?;
    let mixed = mixed
...
fn pruning_expression_partitions_across_row_idx_and_zoned_struct_field() -> VortexResult<()> {
    ...
    let optimized = make_expression_plan(falsifier, source)?.optimize()?;
    insta::assert_snapshot!(optimized.tree_display(), @r"
...
fn chunked_plan_preserves_global_row_index_expressions() -> VortexResult<()> {
    ...
    let plan = ExpressionPlan::new(bound_expression.clone(), child).optimize()?;
    let expression = plan
        .downcast_ref::<ExpressionPlan>()
