...
DEFAULT_DATA_DIR = Path(__file__).resolve().parents[1] / "data" / "huggingface-convert"
...
def parse_args():
    ...
    parser.add_argument("--revision", required=True,
                        help="source branch, tag, or commit (resolved to a commit before conversion)")
    ...
    parser.add_argument("--hub-attempts", type=int, default=5,
    ...
    parser.add_argument("--xet-cache", type=Path, default=DEFAULT_XET_CACHE,
                        help="local Xet chunk/shard cache "
                             "(default: data/huggingface-convert/xet-cache)")
    ...
    parser.add_argument("--upload-repo",
                        help="upload each completed output immediately to this HF dataset repo")
    ...
    parser.add_argument("--upload-prefix", default="",
                        help="destination directory within --upload-repo")
...
def main():
    ...
    if not formats or not set(formats) <= supported_formats:
    ...
    shards = list_shards(api, args.repo, source_commit, args.prefix, args.include,
                         args.filter, args.hub_attempts)
    if args.mode == "all":
        selection = shards
    elif args.mode == "first":
        selection = shards[:args.limit]
    else:
        selection = select_evenly(shards, args.target_size, args.seed)
    uploader = None
    if args.upload_repo:
    ...
    if uploader:
        for fmt in formats:
            ...
            existing_sink_files.update(uploader.existing_files(format_prefix))
